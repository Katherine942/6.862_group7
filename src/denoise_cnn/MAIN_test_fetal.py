...
def main(dataset_type = 'dncnn7', json_path='train_dncnn7.json',model_path = "results/dncnn_epoch1000_snrrand/models/latest_G.pth"):
    ...
    # ----------------------------------------
    '''
    test_folder = 'test_results_fetal'
...
#     current_step = init_iter
    ...
    idx = 0
    logger.info("number of images: {:d}".format(len(test_loader)))
...
#         img_dir = os.path.join(opt['path']['images'], img_name)
        ...
        if idx%1 == 0:
            img_dir = img_dir = os.path.join(opt['path']['images'],'all_imgages')
            util.mkdir(img_dir)
            save_img_path = os.path.join(img_dir, '{:s}_test.png'.format(img_name))
            save_img_org_path = os.path.join(img_dir, '{:s}_org.png'.format(img_name))
            save_img_bm3d_path = os.path.join(img_dir, '{:s}_bm3d.png'.format(img_name))
            util.imsave(E_img, save_img_path)
            util.imsave(H_img, save_img_org_path)
            util.imsave(bm3d_img, save_img_bm3d_path)
            logger.info("saved image: {:s}".format(img_name))
...
#     json_files = ['json/train_dncnn7_epoch1000_snrrand_rl.json','json/train_dncnn7_epoch1000_snrrand_gs.json']
    for dataset_type in ['dncnn7_no_noise']:
        json_pth = 'json/test_fetal.json'
